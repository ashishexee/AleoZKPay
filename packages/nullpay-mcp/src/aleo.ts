...
export const PROGRAM_ID = 'zk_pay_proofs_privacy_v28.aleo';
export const WALLET_PROGRAM_ID = 'zk_pay_proofs_privacy_wallet_v5.aleo';
...
function fieldChunksToString(chunks: string[]): string {
    let result = '';
...
function resolvePaymentMode(
    ...
    fallbackCurrency?: Exclude<Currency, 'ANY'>,
    ...
    tokenType: number;
    amountMicro: bigint;
    originalAmountMicro: bigint;
    functionName: string;
    tokenProgram: string;
    amountSuffix: 'u64' | 'u128';
    originalAmountSuffix: 'u64' | 'u128';
    ...
    const originalAmountMicro = amountMicroFromInvoice ?? BigInt(Math.round(amountMajor * 1_000_000));
    ...
    const originalAmountSuffix: 'u64' | 'u128' = (tokenType === 1 || tokenType === 2) ? 'u128' : 'u64';
    ...
    if (isCrossToken && quoteConvertedMicro) {
        ...
        const payerSuffix: 'u64' | 'u128' = (payerCurrency === 'USDCX' || payerCurrency === 'USAD') ? 'u128' : 'u64';
        return {
            invoiceType,
            tokenType,
...
export function parseBurnerBackupRecord(plaintext: string): ParsedBurnerBackupRecord | null {
    try {
        const getVal = (key: string) => {
            const regex = new RegExp(`(?:${key}|"${key}"):\\s*([\\w\\d\\.]+)`);
            const match = plaintext.match(regex);
...
export async function fetchOwnedBurnerBackupRecords(privateKey: string): Promise<ParsedBurnerBackupRecord[]> {
    const session = await getScannerSession(privateKey);
    const records = await fetchOwnedProgramRecords(session, PROGRAM_ID);
    const parsed: ParsedBurnerBackupRecord[] = [];
...
export async function recoverOnChainWalletBackup(privateKey: string, ownerAddress: string): Promise<RecoveredOnChainWalletBackup | null> {
    const records = await fetchOwnedBurnerBackupRecords(privateKey);
    let passwordOnlyMatch: ParsedBurnerBackupRecord | null = null;
    let fullBurnerMatch: ParsedBurnerBackupRecord | null = null;
...
export async function createSponsoredPaymentAuthorization(args: {
    ...
    const paymentMode = resolvePaymentMode(
        args.invoice, args.amount, args.currency,
        args.payerCurrency, args.quoteConvertedMicro
    );
    ...
    const targetProgramId = paymentMode.isCrossToken ? WALLET_PROGRAM_ID : PROGRAM_ID;
