...
function normalizePaymentCurrency(value?: string): Exclude<Currency, 'ANY'> | undefined {
    if (!value) {
        return undefined;
    }
...
function normalizeInvoiceType(value?: string): InvoiceType {
    if (value === 'multipay' || value === 'donation') return value;
    return 'standard';
}

function tokenTypeLabel(tokenType?: number | null): string {
...
function invoiceTypeLabel(invoiceType?: number | null): string {
    if (invoiceType === 1) return 'multipay';
    if (invoiceType === 2) return 'donation';
    return 'standard';
}

function currencyToTokenType(currency?: Currency | null): number {
...
function linkTokenToCurrency(token?: string | null): Currency | undefined {
    if (!token) {
        return undefined;
    }
    const normalized = token.trim().toLowerCase();
    if (normalized === 'usdcx') return 'USDCX';
    if (normalized === 'usad') return 'USAD';
    if (normalized === 'any') return 'ANY';
    return 'CREDITS';
}

function linkTypeToInvoiceType(type?: string | null): InvoiceType {
...
function parseAmount(value?: string | number | null): number | undefined {
...
function shouldMarkInvoiceSettled(invoiceType?: number | null): boolean {
    return invoiceType !== 1 && invoiceType !== 2;
}

function formatInvoiceSummary(invoice: InvoiceRecord): string {
...
function getAmountSource(invoice: InvoiceRecord): 'record' | 'database' | 'missing' {
...
function buildAmountLookupHint(invoice: InvoiceRecord, hasInvoiceLookupKey: boolean): string {
...
interface ResolvedPayInvoiceContext {
    invoice: InvoiceRecord;
    sessionId?: string;
    source: 'payment_link' | 'invoice_hash';
}

export class NullPayMcpService {
    constructor(
        private readonly backend: NullPayBackendClient,
    ...
    async callTool(name: string, args: Record<string, unknown>): Promise<ToolResult> {
        try {
            if (name === 'login') return await this.login(args as LoginArgs);
    ...
    private async login(args: LoginArgs): Promise<ToolResult> {
        ...
        if (args.create_burner_wallet && !encryptedBurnerKey) {
            const { PrivateKey } = await dynamicImport<any>('@provablehq/sdk');
            ...
            encryptedBurnerAddress = await encryptWithPassword(nextBurnerAddress, password);
    ...
    private async createInvoice(args: CreateInvoiceArgs): Promise<ToolResult> {
        ...
        await this.backend.createInvoiceRow(
            createInvoiceDbRecord({
                invoiceHash,
                merchantAddress,
                amount: invoiceType === 'donation' ? 0 : Number(args.amount),
                title: args.title,
        ...
        const paymentLink = buildPaymentLink(this.publicBaseUrl, {
    ...
    private async payInvoice(args: PayInvoiceArgs): Promise<ToolResult> {
        const wallet = this.resolveWallet(args.wallet);
    ...
    private async getTransactionInfo(args: GetTransactionInfoArgs): Promise<ToolResult> {
        const wallet = this.resolveWallet(args.wallet);
        ...
        if (args.invoice_hash) {
            ...
            const onChain = await getInvoiceStatusData(args.invoice_hash);
        ...
        const invoices = await this.backend.getMerchantInvoices(hashAddress(walletAddress));
    ...
    private resolveWallet(wallet?: WalletPreference): WalletPreference {
    ...
    private async resolveWalletAddress(wallet: WalletPreference): Promise<string> {
        const session = this.sessions.require();
        if (wallet === 'burner') {
            if (!session.encryptedBurnerAddress) {
                throw new Error('Burner wallet address is not stored.');
            }
            return await decryptWithPassword(session.encryptedBurnerAddress, session.password);
        }
        return session.address;
    }

    private async resolveWalletPrivateKey(wallet: WalletPreference): Promise<string> {
    ...
    private async resolveWalletPrivateKeyOptional(wallet: WalletPreference): Promise<string | null> {
        const session = this.sessions.require();
        if (wallet === 'main') {
        ...
        return await decryptWithPassword(session.encryptedBurnerKey, session.password);
    }

    private async resolveInvoiceLookupPrivateKey(wallet?: WalletPreference): Promise<string | null> {
        const session = this.sessions.require();
    ...
    private async resolvePayInvoiceContext(args: PayInvoiceArgs, wallet: WalletPreference): Promise<ResolvedPayInvoiceContext> {
        ...
        if (paymentLink) {
            ...
            if (!invoiceHash && saltFromLink) {
                try {
                    invoiceHash = await waitForInvoiceHash(saltFromLink, 5, 1000);
                } catch {
                    invoiceHash = '';
                }
            }
            ...
            let dbInvoice: InvoiceRecord | null = null;
            try {
            ...
            const effectiveInvoiceType = dbInvoice?.invoice_type ?? invoiceTypeToNumber(linkInvoiceType);
            ...
            const invoice: InvoiceRecord = {
                ...
                amount,
                amount_micro: Math.round(amount * 1_000_000),
    ...
    private async enrichInvoiceIfPossible(invoice: InvoiceRecord, walletPrivateKey: string | null): Promise<InvoiceRecord> {
        try {
            return await enrichInvoiceWithRecordAmount(invoice, walletPrivateKey);
        } catch {
            return invoice;
        }
    }
